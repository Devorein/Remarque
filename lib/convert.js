...
function convert(file_dir, output_dir) {
	let file_name = file_dir.split('\\');
	file_name = file_name[file_name.length - 1];
	let file_path = file_dir + '\\' + file_name;
	const markdown_path = file_path + '.md';
	const markdown_exists = fs.existsSync(markdown_path) && false;
	if (markdown_exists) {
		fs.readFile(markdown_path, 'utf8', (err, data) => {
			...
			htmlElementCreation(document, { type: 'title', textContent: title, parentElem: document.head });
			const content = fs.readFileSync(__dirname + '\\Components\\style.css', 'UTF-8');
			...
			// ? Create the TOC toggle show/hide buttons
			...
			const firstH1Elem = document.querySelector('h1');
			firstH1Elem.id = 'chapter-name';

			const allElem = Array.from(document.querySelector('body').children);
			allElem.shift();

			const contentElem = htmlElementCreation(document, {
				classes      : 'content',
				id           : 'content',
				insertBefore : firstH1Elem,
				children     : [ firstH1Elem, ...allElem ]
			});
			...
			(function createTOC() {
				...
				const DST = [];
				Array.from(document.querySelectorAll('h2,h3,h4,h5,h6')).forEach((topic, index) => {
					...
					while (
						traverser.previousElementSibling &&
						traverser.previousElementSibling.tagName !== `H${level - 1}`
					)
				...
				DST.forEach((elem, index) => {
					for (let i = index + 1; i < DST.length; i++)
						if (DST[i].parentId === elem.id) elem.children.push(DST[i].id);
				});
				...
				DST.forEach((elem) => {
					...
					htmlElementCreation(document, {
						...
						parentElem :
							parentId === 'chapter-name'
								? TOC
								: document.querySelector(`#${parentId}_container + .child-container`)
					});
				});
			})();
			...
			(function headersClasses(document) {
				Array.from(document.querySelectorAll('h2,h3,h4,h5')).forEach((header) => {
					...
					if (isCustom || headerLevel > 2) {
						...
						if (isCustom) {
							header.classList.add('header--subtopic--custom');
							header.classList.add(`header--subtopic${headerLevel}--custom`);
						}
						else {
							header.classList.add(`header--subtopic${headerLevel}--actual`);
							header.classList.add(`header--subtopic--actual`);
						}
					}
				});
			})(document);
			...
			(function addLineNumberClass(document) {
				const code_blocks = [];
				Array.from(document.querySelectorAll('.language-java')).forEach((code) => {
			...
			Array.from(document.querySelectorAll('pre.line-numbers')).forEach((code) => {
				...
				if (code.nextElementSibling.tagName == 'PRE') {
					...
					for (let lines of output_lines) {
						if (output_lines.textContent !== '') {
							...
							if (output_line_numbers && output_line_numbers.index == 0) {
								...
								const line_text = document.createTextNode(
									lines.replace(output_line_numbers + '. ', '')
								);
					...
					for (let i = 0; i < unique_line_numbers.length; i++)
						data_line_text +=
							i + 1 == unique_line_numbers.length
								? unique_line_numbers[i] + ''
								: unique_line_numbers[i] + ',';
			...
			(function explanationHeaderGroup() {
					...
					if (header.nextElementSibling.tagName == 'UL') {
						...
						Array.from(explanationList.children).forEach((explanationList_item) => {
							...
							if (line_priority && line_priority.index == 0) {
								...
								explanationList_item.innerHTML = explanationList_item.innerHTML.replace(
									line_priority[0],
									''
								);
								explanationList_item.insertBefore(
									highlightNumberElem,
									explanationList_item.childNodes[0]
								);
							}
						});
					}
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 184 184" id="theme-icon"><defs><style>.cls-1{fill:#32578e;}.cls-2{fill:#0f315b;}</style></defs><title>Asset 1</title><g  data-name="Layer 2"><g id="Layer_1-2" data-name="Layer 1"><circle class="cls-1" cx="92" cy="92" r="92"/><circle class="cls-2" cx="104.4" cy="33.8" r="12.4"/><circle class="cls-2" cx="42.2" cy="38.1" r="7.5"/><circle class="cls-2" cx="110.1" cy="161.5" r="5.7"/><circle class="cls-2" cx="49.7" cy="81.7" r="5.7"/><circle class="cls-2" cx="49.7" cy="125.6" r="5.7"/><circle class="cls-2" cx="70.6" cy="64" r="7.5"/><circle class="cls-2" cx="22.5" cy="81.7" r="9.9"/><circle class="cls-2" cx="156.6" cy="106.5" r="9.9"/><circle class="cls-2" cx="46.3" cy="145.8" r="9.9"/></g></g></svg>
			...
			(function organizeItems() {
				...
				Array.from(document.querySelectorAll('.topic-container > h3')).forEach((subtopicHeader) => {
					...
					if (subtopicHeader.textContent.includes('➥'))
						subtopicContainer.classList.add('subtopic-container--custom');
					...
					while (
						subtopicContainer.nextElementSibling &&
						subtopicContainer.nextElementSibling.tagName !== 'H3'
					) {
				...
				Array.from(document.querySelectorAll('.subtopic-container > h4')).forEach((subtopicHeader) => {
					...
					while (
						subtopicContainer.nextElementSibling &&
						subtopicContainer.nextElementSibling.tagName !== 'H4'
					) {
						nextElem = subtopicContainer.nextElementSibling;
						subtopicContainer.appendChild(nextElem);
					}
				...
				Array.from(document.querySelectorAll('.subtopicL2-container > h5')).forEach((subtopicHeader) => {
					...
					while (
						subtopicContainer.nextElementSibling &&
						subtopicContainer.nextElementSibling.tagName !== 'H5'
					) {
						nextElem = subtopicContainer.nextElementSibling;
						subtopicContainer.appendChild(nextElem);
					}
			...
			// ? Checks if the element is custom
			...
			fs.writeFile(file_path + '.html', document.documentElement.outerHTML.trim(), 'utf8', (err) => {
				if (err) throw err;
				showSuccessMessage(file_dir);
			});
		});
	}
	else showErrorMessage(markdown_path);
}
