const { getHeaderPosition } = require('./import');
...
window.onload = () => {
	...
	getHeaderPosition(allHeadingPos);
	...
	(function toggleClock() {
		_clockIcon.addEventListener('click', (e) => {
			_clockIcon.classList.toggle('activated');
			const active = _clockIcon.classList.contains('activated');
			_clock.style.display = active ? 'initial' : 'none';
		});
	})();

	(function toggleProgress() {
		_progressIcon.addEventListener('click', (e) => {
			_progressIcon.classList.toggle('activated');
			const active = _progressIcon.classList.contains('activated');
			_progress.style.opacity = active ? '1' : '0';
		});
	})();
	...
	(function addCopyIcon() {
		Array.from(document.querySelectorAll('pre[class*="language"]')).forEach((codeBlock) => {
			...
			if (!codeBlock.classList.contains('language-shell')) createDownloadIcon(codeBlock);
		});
	})();
	...
	function createDownloadIcon(parent) {
