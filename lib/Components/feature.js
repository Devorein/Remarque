  ...
  (function getHeaderPosition() {
	...
	(function usePreviousSetup() {
		...
		if (TOCVisibility === 'shown') {
			...
			document.getElementById('slider-icon').style.left = '350px';
		}
			...
			document.getElementById('slider-icon').style.left = '0px';
		}
  ...
  function toggleTOCElem(value){
    ...
    document.addEventListener('keydown', (e) => {
			if (overlay.style.display === 'block') {
              ...
              entries = findHeaders(allHeadingPos, 'down', topPos)
					}
				}
			}
		});
  ...
  function changeCodeDisplay(value){
	...
	(function addLineHighlight() {
      ...
      const _class = line_highlight.classList[0].split("--");
			highlight_line.classList.add(`${_class[0]}-line--${_class[1]}`);
    });
    Array.from(document.querySelectorAll('.call-line-number,.input-line-number,.error-line-number')).forEach(line_highlight=>{
      const line_number = line_highlight.textContent;
