window.onload = () => {
	...
	(function toggleTOC() {
		_tocToggleIcon.addEventListener('click', (e) => {
			const dropzone = _toc.parentElement.id.split('-')[1];
			_toc.classList.toggle('closed');
			const isClosed = _toc.classList.contains('closed');
			const last_position = window.localStorage.getItem('tocSize');
			if (isClosed && dropzone === 'left') {
				_toc.style.width = `0px`;
				_toc.style.left = `0px`;
				_content.style.width = `100%`;
				_content.style.left = `0%`;
			}
			else if (isClosed && dropzone === 'right') {
				_toc.style.width = `0px`;
				_toc.style.left = `100%`;
				_content.style.width = `100%`;
				_content.style.left = `0%`;
			}
			else if (!isClosed && dropzone === 'left') {
				_toc.style.width = last_position;
				_toc.style.left = `0%`;
				_content.style.width = `calc(100% - ${last_position}px)`;
				_content.style.left = `${last_position}px`;
			}
			else if (!isClosed && dropzone === 'right') {
				_toc.style.width = last_position;
				_toc.style.left = `calc(100% - ${last_position}px)`;
				_content.style.left = `0%`;
				_content.style.width = `calc(100% - ${last_position}px)`;
			}
		});
