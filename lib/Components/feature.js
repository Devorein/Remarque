window.onload = () => {
    ...
    tocInputField.addEventListener("input",(e)=>{
      Object.entries(allTOCElems).forEach(tocElem=>{
        if(tocElem[1].includes(e.target.value)){
          ...
          document.querySelector(`#${tocElem[0]}`).style.display = "none";
      })
    })
  })();
  ...
  (function getallTOCElems(){
		...
		document.getElementById("content").addEventListener('keydown', (e) => {
			if (overlay.style.display === 'block') {
				...
				if (e.shiftKey && e.keyCode == 38) {
					if (e.ctrlKey) entries = findHeaders(allHeadingPos, 'up', topPos, '2');
					else entries = findHeaders(allHeadingPos, 'up', topPos);
				}
				else if (e.shiftKey && e.keyCode == 40) {
					if (e.ctrlKey) entries = findHeaders(allHeadingPos, 'down', topPos, '2');
	...
	(function highlightTOC() {
		document.querySelector('.content').addEventListener('scroll', (e) => {
			...
			timer = setTimeout(function() {
				document.getElementById(previousHighlight + '_container').scrollIntoView({
					behavior : 'smooth'
				});
			}, 150);
	...
	(function addShortcuts() {
      ...
      if(e.shiftKey){
        if (e.code === 'KeyT')
          simulateClick(document.getElementById('theme-icon-container'));
        else if (e.code === 'KeyS') 
          simulateClick(document.getElementById('slider-icon'));
        else if (e.code === 'KeyA') 
          simulateClick(document.getElementById('toggle--show'));
        else if (e.code === 'KeyH') 
          simulateClick(document.getElementById('toggle--hide'));
      }
		});
	})();

	(function addCopyIcon() {
		Array.from(document.querySelectorAll('pre.language-java')).forEach((codeBlock) => {
		...
		function createCopyIcon(parent) {
			...
			parent.parentElement.appendChild(copy_icon);
		}
	})();
	...
	(function addLineHighlight() {
		Array.from(
			...
			highlight_line.classList.add(line_highlight.classList[0]);
		});
	})();
