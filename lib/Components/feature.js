window.onload = () => {
  ...
  _toc.classList.add("dropzone-left");
	...
	(function detectNewFile() {
      ...
      if(tocCodeSearch === "true") _tocCodeSearch.classList.add("code-shown");
      tocCodeToggle(tocCodeSearch);
      if(tocToggleCustom === "true") _tocToggleCustom.classList.add("custom-shown");
      toggleCustom(tocToggleCustom);

			_codeDisplayIcon.textContent = codeDisplay;
			if (lastLink) {
	...
	(function usePreviousSetup() {
		...
		// 	document.querySelector('._toc').classList.remove('slide-left');
		...
		// 	document.querySelector('._toc').classList.add('slide-left');
	...
	(function toggleTOC() {
		_tocToggleIcon.addEventListener('click', (e) => {
			resizeContent(_toc.style.width, true);
		});
	})();
  ...
  function repositionSlider(id){
    if(id === "dropzone-left"){
      ...
      _toc.classList.add("dropzone-right");
      _toc.style.widht = `${parseInt(window.innerWidth - _content.style.width)}px)`
    ...
    const rightDropZone = document.getElementById('dropzone-right');
		if (!toggle) {
			if (lastWidth > 250 && lastWidth < 500 && _toc.classList.contains("dropzone-left")) {
        ...
        rightDropZone.style.width = `${lastWidth}px`;
			}else if(lastWidth < window.innerWidth - 250 && lastWidth > window.innerWidth - 500 && _toc.classList.contains("dropzone-right")){
	...
	// ? _toc _slider toggle
	// (function toggleTOC() {
	// 	_tocToggleIcon.addEventListener('click', (event) => {
	// 		_toc.classList.toggle('slide-left');
	// 		if (_toc.classList.contains('slide-left')) {
	...
	// 	_tocToggleCustom.addEventListener('click', (e) => {
	// 		custom = !custom;
	// 		window.localStorage.setItem('tocToggleCustom', custom);
	// 		allCustomSubtopicsTocs.forEach((customsubtopic) => {
	// 			document
	// 				.querySelector(`span[href='${customsubtopic}']`)
	...
	(function toggleCodeDisplay() {
		_codeDisplayIcon.addEventListener('click', (e) => {
			_codeDisplayIcon.textContent = _codeDisplayIcon.textContent === 'C' ? 'R' : 'C';
			changeCodeDisplay(_codeDisplayIcon.textContent);
			window.localStorage.setItem('codeDisplay', _codeDisplayIcon.textContent);
		});
	})();
  ...
  (function tocCustomToggleListener(){
    _tocToggleCustom.addEventListener("click",(e)=>{
      e.target.classList.toggle("custom-shown");
      const onlyCustom = e.target.classList.contains("custom-shown");
      window.localStorage.setItem("tocToggleCustom",onlyCustom);
      toggleCustom(onlyCustom);
    })
  })();

  function tocCodeToggle(onlyCode){
      Object.entries(allTOCElems).forEach(([id,textContent])=>{
        if(onlyCode.toString() === "true" && !textContent.includes("❰ ❱"))
  ...
  (function repositionLineNumbers(){
    Array.from(document.querySelectorAll('.line-numbers-rows')).forEach(line_number=>{
      const parent = line_number.parentElement;
      parent.parentElement.appendChild(parent.removeChild(line_number));
    });
  })();

  function toggleCustom(onlyCustom){
      Object.entries(allTOCElems).forEach(([id,textContent])=>{
        if(onlyCustom.toString() === "true" && textContent.includes("➥"))
          document.getElementById(id).style.display = "none";
        else
          document.getElementById(id).style.display = "initial";
      })
