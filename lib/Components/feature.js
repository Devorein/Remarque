window.onload = () => {
  ...
  TOC.classList.add("dropzone-left");
	...
	const TOCVisibility = window.localStorage.getItem('TOC-visibility');
	...
	(function detectNewFile() {
		...
		if (!isSameFile) {
			...
			document.getElementById('tocInputField').value = tocSearchValue;
			changeCodeDisplay(codeDisplay);
			document.getElementById('code-display-icon').textContent = codeDisplay;
			if (lastLink) {
	...
	(function addShortcuts() {
		document.addEventListener('keypress', (e) => {
			if (e.shiftKey) {
				...
				else if (e.code === 'KeyC') simulateClick(document.getElementById('code-display-icon'));
			}
		});
	})();
	(function toggleCodeDisplay() {
		const codeDisplayIcon = document.getElementById('code-display-icon');
	...
	(function addLineHighlight() {
		Array.from(
			...
			for (let i = 0; i < highlight_lines.length; i++)
				if (!highlight_lines[i].className.includes('highlight-line--')) {
		...
		Array.from(
			...
			const line_number = line_highlight.textContent;
			const code = line_highlight.parentElement.parentElement.previousElementSibling.querySelector(
				`div[data-range='${line_number}']`
			);
			code.classList.add(`code-${line_highlight.classList[0]}`);
		});
	})();
