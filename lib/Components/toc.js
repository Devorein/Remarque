...
function toc_linkClickScrollLink() {
	Array.from(document.querySelectorAll('.toc--content')).forEach(_link => {
		_link.addEventListener('click', e => {
			...
			window.localStorage.setItem('lastFilePath', window.location.pathname);
		});
	});
}

function toggleTOCElem() {
	const searchIgnoreCapIcon = document.getElementById('searchIgnoreCap').classList.contains('activated');
	const tocCodeSearch = document.getElementById('tocCodeSearch').classList.contains('activated');
	const tocToggleCustom = document.getElementById('tocToggleCustom').classList.contains('activated');
	...
	let _shown_items = _toc_items.filter(_toc_item => {
		if (searchIgnoreCapIcon) {
			if (_toc_item.textContent.toLowerCase().includes(searchValue.toLowerCase())) {
				_toc_item.style.display = 'inherit';
				return _toc_item;
			}
