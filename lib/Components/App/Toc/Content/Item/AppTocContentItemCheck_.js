class AppTocContentItemCheck {
	static _checkOnLoad() {
		...
		SETTINGS['app_toc__clickedItems'].forEach(index => {
			_checkmarks[index - 1].firstElementChild.classList.add('scaleUp');
		...
		Array.from(document.querySelectorAll('.checkmark')).forEach((_checkmark, index) => {
			_checkmark.addEventListener('click', e => {
				if (!e.shiftKey) {
					...
					if (checked) _checkmark.firstElementChild.classList.add('scaleUp');
					else _checkmark.firstElementChild.classList.remove('scaleUp');
					...
					// 				_checkmark.firstElementChild.classList.add("scaleUp")
					// 				this.addToSelectedToc(_checkmark);
					// 			}
					// 		}
					// 		else if (
		...
		while (_parent && !_parent.classList.contains('toc--L1-item-container')) {
			if (_parent) {
				...
				if (
					...
					_parentCheckmark.firstElementChild.classList.add('scaleUp');
					...
					_parentCheckmark.firstElementChild.classList.remove('scaleUp');
	...
	static checkForTOCChildrenSelection(_checkmark) {
		...
		_tocItems.forEach(_tocItem => {
			...
			if (checked && !_childChechmark.classList.contains('checked')) {
				...
				_childChechmark.firstElementChild.classList.add('scaleUp');
				...
				_childChechmark.firstElementChild.classList.remove('scaleUp');
				this.addToSelectedToc(_childChechmark);
			}
		});
	}
