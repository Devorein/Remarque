const { simulateClick, createElementFromHTML, switchClass } = require('./utility');
...
// 🔨 Refactor for left right toc position
function tocToggleHandler() {
	...
	_tocToggleIcon.addEventListener('click', e => {
		...
		if (tocPosition === 'left') {
			if (activated) {
				window.localStorage.setItem('tocSize', 0);
				_toc.parentElement.style.width = '0px';
				_content.style.width = `calc(100% - 30px)`;
				_content.style.left = `30px`;
				_toc.querySelector('#tocSearchContainer').style.display = 'none';
			}
			else {
				window.localStorage.setItem('tocSize', 450);
				const tocSize = window.localStorage.getItem('tocSize');
				_toc.parentElement.style.width = `${tocSize}px`;
				_content.style.width = `calc(100% - ${parseInt(tocSize) + 30}px)`;
				_content.style.left = `${parseInt(tocSize) + 30}px`;
				_toc.querySelector('#tocSearchContainer').style.display = 'flex';
			}
		}
...
function clock_progressClickToggleVisibility() {
	[ 'clock', 'progress' ].forEach(icon => {
		...
		_icon.addEventListener('click', e => {
			...
			_target.style.opacity = _icon.classList.toggle('activated') ? '1' : '0';
			window.localStorage.setItem(icon, _icon.classList.contains('activated'));
		});
	});
}
