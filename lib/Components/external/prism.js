	...
	Prism = (function(u) {
		...
		if ((e && ((C.filename = e.src), e.hasAttribute('data-manual') && (C.manual = !0)), !C.manual)) {
			...
			'loading' === r || ('interactive' === r && e && e.defer)
				...
				: window.requestAnimationFrame ? window.requestAnimationFrame(t) : window.setTimeout(t, 16);
		}
		return C;
	})(_self);
...
!(function(s) {
	...
	(s.languages.csharp = s.languages.extend('clike', {
		...
		s.languages.insertBefore('csharp', 'class-name', {
			...
			'type-expression': {
				pattern: t('(\\b(?:default|typeof|sizeof)\\s*\\(\\s*)(?:[^()\\s]|\\s(?!\\s*\\))|<<0>>)*(?=\\s*\\))', [
					b
				]),
				lookbehind: !0,
				alias: 'class-name',
			...
			'type-list': {
				...
				lookbehind: !0,
				inside: { keyword: p, 'class-name': { pattern: RegExp(y), inside: w }, punctuation: /,/ }
			},
			preprocessor: {
				pattern: /(^\s*)#.*/m,
				lookbehind: !0,
				...
				if (0 != t.length) {
					...
					i.sort(function(e, a) {
						...
						i.forEach(function(e) {
							e.open
								? ((e.element.className += ' brace-level-' + (s % 12 + 1)), s++)
								: ((s = Math.max(0, s - 1)), (e.element.className += ' brace-level-' + (s % 12 + 1)));
						});
				}
			}
		});
