...
function toc_item_checkboxClickSelectItem() {
	Array.from(document.querySelectorAll('.checkmark')).forEach(_checkmark => {
		_checkmark.addEventListener('click', e => {
			...
			const _childContainer = _checkmark.parentElement.nextElementSibling;
			if (_childContainer) {
				[ ..._childContainer.children ].forEach(_childContainerItem => {
					const _child = _childContainerItem.firstElementChild.firstElementChild;
					if (!_child.classList.contains('checked') && checked) simulateClick(_child);
					else if (_child.classList.contains('checked') && !checked) simulateClick(_child);
				});
			}
			let _parent = _checkmark.parentElement.parentElement.parentElement.parentElement;
			checkForTOCSelection(_parent, _checkmarkIcon);
			while (!_parent.classList.contains('toc--L1-item-container')) {
...
function checkForTOCSelection(_parent, _checkmarkIcon) {
