...
function toc_item_checkboxClickSelectItem() {
	Array.from(document.querySelectorAll('.checkmark')).forEach(_checkmark => {
		_checkmark.addEventListener('click', e => {
			...
			addToSelectedToc(checked, _checkmark);
			...
			_tocItems.forEach(_tocItem => {
				...
				if (checked && !_childChechmark.classList.contains('checked')) {
					...
					addToSelectedToc(checked, _childChechmark);
				}
			...
			checkForTOCParentSelection(_parent, _checkmarkIcon);
...
function addToSelectedToc(purpose, _item) {
...
function checkForTOCParentSelection(_parent, _checkmarkIcon) {
	if (_parent) {
		...
		if (
			...
			addToSelectedToc(true, _parentCheckmark);
		}
		else if (
			...
			addToSelectedToc(false, _parentCheckmark);
		}
	}
}
