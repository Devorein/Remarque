...
function toc_item_checkboxClickSelectItem() {
	...
	Array.from(_checkmarks).forEach((_checkmark, index) => {
		_checkmark.addEventListener('click', e => {
				...
				for (let i = index; i >= 0; i--) {
					...
					if (checked) {
						...
						if (!_parent) {
							...
							_parent &&
							_parent.querySelectorAll('.toc-item').length === _parent.querySelectorAll('.checked').length
						) {
							_checkmark.classList.add('checked');
							if (!_checkmark.firstElementChild) {
								_checkmark.appendChild(_checkmarkIcon.cloneNode(true));
			...
			_toc.dispatchEvent(new CustomEvent('itemClicked'));
		});
	});
}

function tocCheckedItemHandler() {
	_toc.addEventListener('itemClicked', e => {
