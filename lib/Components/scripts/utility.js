...
function calculateTOC_Content() {
	...
	_clockContainer.style.left = `${parseInt(_content.style.left.replace('px', '')) + 10}px`;
	_progressContainer.style.left = `${parseInt(_content.style.left.replace('px', '')) + 10}px`;
	_content.style.width = `calc(100% - ${tocSize}px)`;
	_progressContainer.style.width = _content.style.width;
}
