...
function checkForZenMode() {
	...
	const zenModeToggle = window.localStorage.getItem('zenModeToggle');
	if (zenModeToggle === 'true') {
		if (tocToggle === 'false' || clock === 'true' || progress === 'true' || !document.fullscreenElement) {
			_zenModeToggleIcon.classList.remove('activated');
			window.localStorage.setItem('zenModeToggle', false);
		}
	}
	else if (zenModeToggle === 'false') {
		if (tocToggle === 'true' && clock === 'false' && progress === 'false' && document.fullscreenElement) {
			_zenModeToggleIcon.classList.add('activated');
			window.localStorage.setItem('zenModeToggle', true);
		}
	}
}
...
function tocToggleHandler() {
	...
	_tocToggleIcon.addEventListener('click', e => {
		...
		_toc.parentElement.classList.add('transition');
		_content.classList.add('transition');
		setTimeout(() => {
			_toc.parentElement.classList.remove('transition');
			_content.classList.remove('transition');
		}, 500);
...
function zen_modeIClickTogglesZenMode() {
	_zenModeToggleIcon.addEventListener('click', e => {
		const activated = _zenModeToggleIcon.classList.toggle('activated');
		window.localStorage.setItem('zenModeToggle', activated);
		zen_modeHandler();
	});
}
function zen_modeHandler() {
	const tocToggle = window.localStorage.getItem('tocToggle');
	const clock = window.localStorage.getItem('clock');
	const progress = window.localStorage.getItem('progress');
	const zenMode = window.localStorage.getItem('zenModeToggle');
	if (zenMode.toString() === 'true') {
		if (tocToggle.toString() === 'false') simulateClick(_tocToggleIcon);
		if (clock.toString() === 'true') simulateClick(_clockIcon);
		if (progress.toString() === 'true') simulateClick(_progressIcon);
		...
		if (tocToggle.toString() === 'true') simulateClick(_tocToggleIcon);
		if (clock.toString() === 'false') simulateClick(_clockIcon);
		if (progress.toString() === 'false') simulateClick(_progressIcon);
...
function switchToSettingsPage() {
	_settingsPage.addEventListener('click', e => {
		_APP.style.left = '100%';
		_SETTINGS.style.left = '0%';
	});
}

function groupsPageIconHandler() {
	_groupsIcon.addEventListener('click', e => {
		_APP.style.left = '100%';
...
function switchToGroupsPage() {
	_groupsPageCancel.addEventListener('click', e => {
		_APP.style.left = '0%';
		_GROUPS.style.left = '-100%';
	});
}
