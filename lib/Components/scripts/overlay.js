...
function customblockClickAppendsOverlay() {
	Array.from(document.querySelectorAll('.groupContainer .text-block')).forEach((_customBlock, index, arr) => {
...
function overlay_moveIClickSwitchesBlock() {
	const _block_container_childs = Array.from(document.querySelectorAll('.groupContainer .text-block'));
...
function overlay_unique_moveIClickSwitchesBlock() {
	...
	const _custom_blocks = Array.from(document.querySelectorAll('.groupContainer .text-block'));
	[ _overlayPrevUniqueIcon, _overlayNextUniqueIcon ].forEach(_icon => {
		_icon.addEventListener('click', e => {
			...
			if (_icon.id === _overlayNextUniqueIcon.id) {
				let i = lastBlockSelected === _custom_blocks.length - 1 ? 0 : lastBlockSelected + 1;
				...
				let i = lastBlockSelected === 0 ? _custom_blocks.length - 1 : lastBlockSelected - 1;
				for (; i >= 0; ) {
					const new_start_str = _custom_blocks[i].textContent.codePointAt(0);
					if (current_start_str !== new_start_str) {
						target_index = i;
