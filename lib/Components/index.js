...
const { displayTime, play_pause_clockListener, restart_clockIHandler } = require('./clock');
...
window.onload = () => {
	...
	toggleFileSystem();
	tocToggleHandler();
	if (window.localStorage.getItem('zenModeToggle').toString() === 'true') simulateClick(_zenModeToggleIcon);
};
