window.onload = () => {
  ...
  const tocInputField = document.getElementById('tocInputField');
  const _tocCodeSearch = document.getElementById('tocCodeSearch');
  const codeDisplayIcon = document.getElementById('codeDisplayIcon');

  let allCustomSubtopicsTocs = [];
  TOC.classList.add("dropzone-left");
	...
	const TOCCustom = window.localStorage.getItem('TOC-custom') === 'false' ? false : true;
	...
	(function detectNewFile() {
		...
		if (!isSameFile) {
			...
			tocInputField.value = tocSearchValue;
      changeCodeDisplay(codeDisplay);
      if(tocCodeSearch) _tocCodeSearch.classList.add("code-shown");
      tocCodeToggle(tocCodeSearch);

			codeDisplayIcon.textContent = codeDisplay;
			if (lastLink) {
	...
	(function scrollExtremeToggle() {
		Array.from(document.getElementById('page-extreme-icon').children).forEach((icon) => {
			icon.addEventListener('click', (e) => {
				document.getElementById(icon.id.split('--')[1]).scrollIntoView({
					behavior : 'smooth'
				});
			});
		});
	...
	(function usePreviousSetup() {
		...
		// if (TOCVisibility === 'shown') {
		// 	document.querySelector('.TOC').classList.remove('slide-left');
		// 	document.querySelector('.content').style.width = 'calc(100% - 350px)';
		// 	document.querySelector('.content').style.left = '350px';
		// }
		// else if (TOCVisibility === 'hidden') {
		// 	document.querySelector('.TOC').classList.add('slide-left');
		// 	document.querySelector('.content').style.width = '100%';
		// 	document.querySelector('.content').style.left = '0px';
		// }

		if (TOCCustom)
			allCustomSubtopicsTocs.forEach((customsubtopic) => {
				document
					.querySelector(`span[href='${customsubtopic}']`)
	...
	(function addTOCSearch() {
		tocInputField.addEventListener('input', (e) => {
	...
	(function toggleTOC() {
		...
		sliderIcon.addEventListener('click', (e) => {
			resizeContent(TOC.style.width, true);
		});
	})();
  let dragged;
  ...
  function repositionSlider(id){
    if(id === "dropzone-left"){
      TOC.classList.remove("dropzone-right");
      TOC.classList.add("dropzone-left");
      TOC.style.left = "0px";
      CONTENT.style.left = `${parseInt(TOC.style.width)}px`
      slider.style.left = "100%";
    }else if(id==="dropzone-right"){
      CONTENT.style.left = "0px";
      TOC.classList.remove("dropzone-left");
      TOC.classList.add("dropzone-right");
      TOC.style.widht = `${parseInt(window.innerWidth - CONTENT.style.width)}px)`
      slider.style.left = "0%";
    }
  }
	(function dragSlider() {
	...
	function resizeContent(lastWidth, toggle) {
    const leftDropZone = document.getElementById('dropzone-left');
    const rightDropZone = document.getElementById('dropzone-right');
		if (!toggle) {
			if (lastWidth > 250 && lastWidth < 500 && TOC.classList.contains("dropzone-left")) {
        TOC.style.width = `${lastWidth}px`;
        TOC.style.left = "0px";
				CONTENT.style.width = `calc(100% - ${parseInt(TOC.style.width)}px)`;
        CONTENT.style.left = `${parseInt(TOC.style.width)}px`;
        leftDropZone.style.width = `${lastWidth}px`;
        rightDropZone.style.width = `${lastWidth}px`;
			}else if(lastWidth < window.innerWidth - 250 && lastWidth > window.innerWidth - 500 && TOC.classList.contains("dropzone-right")){
        TOC.style.width = `${parseInt(window.innerWidth - lastWidth)}px`;
        TOC.style.left = `calc(100% - ${parseInt(TOC.style.width)}px)`;
				CONTENT.style.width = `calc(100% - ${parseInt(TOC.style.width)}px)`;
        CONTENT.style.left = `0px`;
        const calculatedWidth = Math.abs(parseInt(window.innerWidth - lastWidth));
        leftDropZone.style.width = `${calculatedWidth}px`;
        rightDropZone.style.width = `${calculatedWidth}px`;
      }
		}
		else {
			if (lastWidth == "0px") lastWidth = 350;
      else lastWidth = 0;
			TOC.style.width = `${lastWidth}px`;
			content.style.width = `calc(100% - ${parseInt(TOC.style.width)}px)`;
			content.style.left = `${parseInt(TOC.style.width)}px`;
	...
	(function toggleTOC() {
		...
		slider.addEventListener('click', (event) => {
			TOC.classList.toggle('slide-left');
			if (TOC.classList.contains('slide-left')) {
				window.localStorage.setItem('TOC-visibility', 'hidden');
				document.querySelector('.content').style.width = '100%';
				document.querySelector('.content').style.left = '0px';
			}
			else {
				window.localStorage.setItem('TOC-visibility', 'shown');
				document.querySelector('.content').style.width = 'calc(100% - 350px)';
				document.querySelector('.content').style.left = '350px';
			}
		});
	...
	function appendToOverlay(textBlock) {
		...
		overlay.appendChild(clonedtextBlock);
	}
	...
	(function toggleCustom() {
		const customTOCIcon = document.querySelector('.icon--custom-toggle');
		let custom = TOCCustom;
		customTOCIcon.addEventListener('click', (e) => {
			custom = !custom;
			window.localStorage.setItem('TOC-custom', custom);
			allCustomSubtopicsTocs.forEach((customsubtopic) => {
				document
					.querySelector(`span[href='${customsubtopic}']`)
	...
	(function addShortcuts() {
		document.addEventListener('keypress', (e) => {
			if (e.shiftKey) {
				...
				else if (e.code === 'KeyA') simulateClick(document.getElementById('toggle--show'));
				else if (e.code === 'KeyH') simulateClick(document.getElementById('toggle--hide'));
				else if (e.code === 'KeyC') simulateClick(codeDisplayIcon);
			}
		});
	})();
	(function toggleCodeDisplay() {
		codeDisplayIcon.addEventListener('click', (e) => {
			codeDisplayIcon.textContent = codeDisplayIcon.textContent === 'C' ? 'R' : 'C';
			changeCodeDisplay(codeDisplayIcon.textContent);
			window.localStorage.setItem('codeDisplay', codeDisplayIcon.textContent);
		});
	})();
	...
	(function addCopyIcon() {
		...
		function createCopyIcon(parent) {
			...
			copy_icon.addEventListener('click', () => {
				copyToClipboard(parent.textContent.replace('📄', '').replace('{', '{ \n'));
			});
  ...
  // ? only show code in TOC

  (function tocCodeToggleListener(){
    _tocCodeSearch.addEventListener("click",(e)=>{
      e.target.classList.toggle("code-shown");
      const onlyCode = e.target.classList.contains("code-shown");
      window.localStorage.setItem("tocCodeSearch",onlyCode);
      tocCodeToggle(onlyCode);
    })
  })();

  function tocCodeToggle(onlyCode){
    console.log(onlyCode);
    Object.entries(allTOCElems).forEach(([id,textContent])=>{
      if(onlyCode && !textContent.includes("❰ ❱"))
