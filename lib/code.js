...
function distinguishCodeBlock(_code) {
	...
	if (
		...
		__code.innerHTML = hljs.highlight(__code.classList[0], __code.textContent).value;
	}
}
...
function groupCodeBlocks(_code) {
	...
	while (_traverser.tagName === 'PRE' || _traverser.tagName === 'UL') {
		...
		if (_traverser.children[0].classList.contains('language-shell')) {
			...
			_traverser.style.zIndex = '-1';
			_traverser.style.opacity = '0';
			const ___code = _traverser.querySelector('code');
			___code.innerHTML = hljs.highlight(___code.classList[0], ___code.textContent).value;
...
function addOutputHighlight(_parent, _code) {
	...
	for (let line of output_lines) {
		...
		if (line_number) {
			...
			if (line.includes('=>'))
				innerHTML += `<div class="output-block-line">${parseFC(line_number[0], _code)}${parseReg(
					line_number[0],
					_code
				)}${text}</div>`;
			else innerHTML += `<div class="output-block-line">${parseReg(line_number[0], _code)}${text}</div>`;
		}
		else innerHTML += `<div class="output-block-line">${line}</div>`;
	}
...
function addCodeHighlight(line, _code, purpose, group) {
	...
	if (line.includes('-')) {
		...
		stop = parseInt(stop);
		const top = line_height * start - start / 2 - 4;
		_line.style.top = `${top}px`;
		const bottom = line_height * stop - stop / 2 - 4;
		_line.style.height = `${bottom - top + 17}px`;
	}
	else {
		const _line = htmlElementCreation({
			parentElem : _code,
