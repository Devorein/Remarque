...
function addOutputHighlight(document, output) {
	...
	for (let line of output_lines) {
		if (output_lines.textContent !== '') {
			...
			if (matched_line) {
				const local_line_numbers = matched_line[0].split(',');
				local_line_numbers.forEach((line_number) => {
					const line_purpose = detectLinePurpose(line_number);
					const highlight_lines = line_number.replace(/\./g, '');
					...
					outputLineDivElems.push(
						htmlElementCreation(document, {
							...
							children : [
								...
								document.createTextNode(line.substring(matched_line[0].length + 1))
							]
						})
					);
				});
