...
function addOutputHighlight(document, output) {
	...
	for (let line of output_lines) {
		if (line !== '') {
			...
			if (matched_line) {
				...
				if (containsFC)
					line_numbers = matched_line[0].substring(containsFC[0].length).replace('.', '').split(',');
				else line_numbers = matched_line[0].replace('.', '').split(',');
	...
	Array.from(new Set(all_highlight_lines)).forEach((line_number) => {
		data_line_text += line_number + ',';
	});

	data_line_text = data_line_text.substring(0, data_line_text.length - 1);
	...
	if (fctext) {
		fctext.match(/\d/g).forEach((fcline, index) => {
			fnSpans.push(
				htmlElementCreation(document, {
					type        : 'span',
					classes     : [ 'call-line-number', `${index + 1}_call-line-number` ],
					textContent : fcline
				})
			);
		});
